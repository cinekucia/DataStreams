...
from SmartSRPDW import SmartSRPDW
...
def run_evaluation(arff_file_path: str, output_dir: str, \
    ...
    """
    Run a probability-aware prequential evaluation on a single ARFF file using SmartSRPDW,
    ...
    # 3. Instantiate SmartSRPDW
    base_tree = tree.HoeffdingTreeClassifier(
        grace_period=50,
        delta=0.01,
        nominal_attributes=nominal_attrs or None
    )
    model_smart = SmartSRPDW(
        model=base_tree,
        n_models=10,
        max_models=15,
        monitor_window=1000,
...
if __name__ == "__main__":
    data_dir = "./data"
    output_dir = "./SmartSRPDW_FINAL_results"
    ...
    if all_metrics:
        ...
        combined_csv = os.path.join(output_dir, f"metrics_SmartSRPDW_10_15_all_streams.csv")
