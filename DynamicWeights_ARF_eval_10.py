...
from ARFDW import ARFClassifierDynamicWeights
...
def run_evaluation(arff_file_path: str, output_dir: str, \
                   window_size: int = 1000, max_instances: int = 1_000_000_000_000,
                   optimise: bool = False, progress_bar: bool = True) -> pd.DataFrame:
    """
    Run a probability-aware prequential evaluation on a single ARFF file using ARFClassifierDynamicWeights,
    ...
    # 3. Instantiate ARFClassifierDynamicWeights model
    arf = ARFClassifierDynamicWeights(
        n_models=10,
        max_features="sqrt",
        lambda_value=6,
        seed=42,
    ...
    model_name = "ARF_dynamic_weights"
...
if __name__ == "__main__":
    ...
    dataset_files = [
        ...
        "covtype.arff","electricty.arff","airlines.arff","internet_ads.arff","kdd99.arff"
    ]
    ...
    if all_metrics:
        ...
        combined_csv = os.path.join(output_dir, f"metrics_ARF_dynamic_weights_all_streams.csv")
